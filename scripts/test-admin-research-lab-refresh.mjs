...
try {
  ...
  assert.doesNotMatch(componentSource, /Loading operational health\.\.\./)
  assert.match(componentSource, /if \(viewMode === 'requests'\)[\s\S]{0,220}mode=shell/)
  assert.match(componentSource, /const res = await fetch\('\/api\/admin\/research-lab',/)
