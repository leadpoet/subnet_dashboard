...
`
  ...
  assert.match(metagraphUiSource, /Validator and epoch data refresh together every 30s/)
  assert.match(metagraphUiSource, /Data refreshed \{lastRefreshAt/)
  assert.doesNotMatch(metagraphUiSource, /EPOCH_REFRESH_INTERVAL_MS/)
  ...
  assert.match(validatorWeightsDeploy, /grep -Fq '__incident_delivery_epochs__'/)
  assert.match(validatorWeightsDeploy, /grep -Fq '"delivery_failed"'/)
  assert.match(validatorWeightsDeploy, /grep -Fq '__heartbeat__'/)
