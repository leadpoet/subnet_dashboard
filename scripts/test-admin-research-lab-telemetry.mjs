...
try {
  ...
  assert.match(routeSource, /ADMIN_LAB_OVERVIEW_STALE_MS = 24 \* 60 \* 60_000/)
  ...
  const instrumentationSource = await readFile(resolve('src/instrumentation.ts'), 'utf8')
  assert.match(instrumentationSource, /GET\(new NextRequest\('http:\/\/localhost\/api\/admin\/research-lab'\)\)/)
  assert.match(instrumentationSource, /overview cache warm-up complete/)
  assert.match(routeSource, /const mode = request\.nextUrl\.searchParams\.get\('mode'\)/)
  assert.match(routeSource, /mode === 'shell'/)
  assert.match(routeSource, /'X-Admin-Lab-View': 'shell'/)
  ...
  assert.match(adminComponentSource, /Live · 30s overview/)
  assert.match(adminComponentSource, /Request index loaded · 15s active run/)
